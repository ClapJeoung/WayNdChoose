...
public class maptext : MonoBehaviour
{
 ...
 city_faith_max = 0, city_faith_aver = 0, city_culture_max = 0, city_culture_aver = 0;
    int castle_wealth_max = 0, castle_wealth_aver = 0,
 castle_faith_max = 0, castle_faith_aver = 0, castle_culture_max = 0, castle_culture_aver = 0;
    ...
    for (int a = 0; a < 1000; a++)
      ...
      for (int i = 0; i < _currentmap.TownCount; i++)
      {
        ...
        if (_currentmap.Wealth_town[i] <= Mydeg.Town_Wealth_Low) _town_wealth_low++;
        else if (_currentmap.Wealth_town[i] <= Mydeg.Town_Wealth_Middle) _town_wealth_middle++;
        else _town_wealth_high++;
        if (_currentmap.Faith_town[i] <= Mydeg.Town_Faith_Low) _town_faith_low++;
        else if (_currentmap.Faith_town[i] <= Mydeg.Town_Faith_Middle) _town_faith_middle++;
      ...
      for (int i = 0; i < _currentmap.CityCount; i++)
      {
        ...
        if (_currentmap.Wealth_city[i] <= Mydeg.City_Wealth_Low) _city_wealth_low++;
        else if (_currentmap.Wealth_city[i] <= Mydeg.City_Wealth_Middle) _city_wealth_middle++;
        else _city_wealth_high++;
        if (_currentmap.Faith_city[i] <= Mydeg.City_Faith_Low) _city_faith_low++;
        else if (_currentmap.Faith_city[i] <= Mydeg.City_Faith_Middle) _city_faith_middle++;
        else _city_faith_high++;
        if (_currentmap.Culture_city[i] <= Mydeg.City_Culture_Low) _city_culture_low++;
        else if (_currentmap.Culture_city[i] <= Mydeg.City_Culture_Middle) _city_culture_middle++;
      ...
      for (int i=0;i<_currentmap.CastleCount; i++)
      {
        ...
        if (_currentmap.Wealth_castle[i] <= Mydeg.Caslte_Wealth_Low) _castle_wealth_low++;
        else if (_currentmap.Wealth_castle[i] <= Mydeg.Caslte_Wealth_Middle) _castle_wealth_middle++;
        else _castle_wealth_high++;
        if (_currentmap.Faith_castle[i] <= Mydeg.Caslte_Faith_Low) _castle_faith_low++;
        else if (_currentmap.Faith_castle[i] <= Mydeg.Caslte_Faith_Middle) _castle_faith_middle++;
        else _castle_faith_high++;
        if (_currentmap.Culture_castle[i] <= Mydeg.Caslte_Culture_Low) _castle_culture_low++;
        else if (_currentmap.Culture_castle[i] <= Mydeg.Caslte_Culture_Middle) _castle_culture_middle++;
  ...
  $"{_townwealthdata}\n" +
          $"low : {_town_wealth_low,-4}   middle : {_town_wealth_middle,-4}   high : {_town_wealth_high}\n\n" +
  $"신앙 최대 : {town_faith_max}   신앙 평균 : {town_faith_aver}\n" +
     $"{_townfaithdata}\n" +
       $"low : {_town_faith_low,-4}   middle : {_town_faith_middle,-4}   high : {_town_faith_high}\n" +
  $"───────────────────────────────────────────────────────────\n" +
      $"도시 검사  도시 총 개수 : {_citycount}\n\n" +
  $"부 최대 : {city_wealth_max}   부 평균 : {city_wealth_aver}\n" +
     $"{_citywealthdata}\n" +
    $"low : {_city_wealth_low,-4}   middle : {_city_wealth_middle,-4}   high : {_city_wealth_high}\n\n" +
  $"신앙 최대 : {city_faith_max}   신앙 평균 : {city_faith_aver}\n" +
      $"{_cityfaithdata}\n" +
           $"low : {_city_faith_low,-4}   middle : {_city_faith_middle,-4}   high : {_city_faith_high}\n\n" +
      $"문화 최대 : {city_culture_max}   문화 평균 : {city_culture_aver}\n" +
      $"{_cityculturedata}\n" +
          $"low : {_city_culture_low,-4}   middle : {_city_culture_middle,-4}   high : {_city_culture_high}\n" +
 $"───────────────────────────────────────────────────────────\n" +
      $"성채 검사  성채 총 개수 : {_castlecount}\n\n" +
  $"부 최대 : {castle_wealth_max}   부 평균 : {castle_wealth_aver}\n" +
       $"{_castlewealthdata}\n" +
     $"low : {_castle_wealth_low,-4}   middle : {_castle_wealth_middle,-4}   high : {_castle_wealth_high}\n\n" +
  $"신앙 최대 : {castle_faith_max}   신앙 평균 : {castle_faith_aver}\n" +
     $"{_castlefaithdata}\n" +
          $"low : {_castle_faith_low,-4}   middle : {_castle_faith_middle,-4}   high : {_castle_faith_high}\n\n" +
    ...
    #endregion
    #region 정착지 정보
    _newmapdata.BottomMapCode = _mc_b;
        _newmapdata.TopMapCode = _mc_t;
        _newmapdata.BottomTileCode = _tc_b;
        _newmapdata.TopTileCode = _tc_t;
        _newmapdata.RotCode = _r;

        _newmapdata.TownCount = _towns.Count;
        _newmapdata.Town_Pos = _towns.ToArray();
        ...
        _newmapdata.CityCount = _citycount;
        _newmapdata.City_Pos = _citytemp.ToArray();

        _newmapdata.CastleCount = _castles.Count;
        ...
        _newmapdata.Castle_Pos = _castletemp.ToArray();
    ...
    _newmapdata.Isriver_town = new bool[_newmapdata.TownCount];
    _newmapdata.Isforest_town=new bool[_newmapdata.TownCount];
    _newmapdata.Ismine_town= new bool[_newmapdata.TownCount];
    _newmapdata.Ismountain_town= new bool[_newmapdata.TownCount];
    _newmapdata.Issea_town = new bool[_newmapdata.TownCount];
        _newmapdata.Wealth_town = new int[_newmapdata.TownCount];
        _newmapdata.Faith_town = new int[_newmapdata.TownCount];
        _newmapdata.Culture_town = new int[_newmapdata.TownCount];
        _newmapdata.Science_town = new int[_newmapdata.TownCount];

    _newmapdata.Isriver_city = new bool[_newmapdata.CityCount];
    _newmapdata.Isforest_city = new bool[_newmapdata.CityCount];
    _newmapdata.Ismine_city = new bool[_newmapdata.CityCount];
    _newmapdata.Ismountain_city = new bool[_newmapdata.CityCount];
    _newmapdata.Issea_city = new bool[_newmapdata.CityCount];
    _newmapdata.Wealth_city = new int[_newmapdata.CityCount];
    _newmapdata.Faith_city = new int[_newmapdata.CityCount];
    _newmapdata.Culture_city = new int[_newmapdata.CityCount];
    _newmapdata.Science_city = new int[_newmapdata.CityCount];

    _newmapdata.Isriver_castle = new bool[_newmapdata.CastleCount];
    _newmapdata.Isforest_castle = new bool[_newmapdata.CastleCount];
    _newmapdata.Ismine_castle = new bool[_newmapdata.CastleCount];
    _newmapdata.Ismountain_castle = new bool[_newmapdata.CastleCount];
    _newmapdata.Issea_castle = new bool[_newmapdata.CastleCount];
    _newmapdata.Wealth_castle = new int[_newmapdata.CastleCount];
    _newmapdata.Faith_castle = new int[_newmapdata.CastleCount];
    _newmapdata.Culture_castle = new int[_newmapdata.CastleCount];
    _newmapdata.Science_castle = new int[_newmapdata.CastleCount];
    ...
    //마을 개수대로 검사
    for(int i = 0; i < _newmapdata.TownCount; i++)
    {
      _poslist.Clear();
      _poslist.Add(_newmapdata.Town_Pos[i]);
      List<Vector3Int> _temp = new List<Vector3Int>();
      foreach (var _asdf in _poslist) _temp.Add(_asdf);
      _poses.Add(_temp);
    }
    _settles = ClassifySettle(_poses,_desertcount_town);
    for(int i = 0; i < _settles.Count; i++)
    {
      _newmapdata.Isriver_town[i] = _settles[i].IsRiver;
      _newmapdata.Isforest_town[i] = _settles[i].IsForest;
      _newmapdata.Ismine_town[i] = _settles[i].IsMine;
      _newmapdata.Ismountain_town[i] = _settles[i].IsMountain;
      _newmapdata.Issea_town[i]= _settles[i].IsSea;
      _newmapdata.Wealth_town[i] = _settles[i].Wealth;
      _newmapdata.Faith_town[i] = _settles[i].Faith;
      _newmapdata.Culture_town[i] = _settles[i].Culture;
      _newmapdata.Science_town[i] = _settles[i].Science;
    }
    _poses.Clear();
    _poslist.Clear();
    //도시 2개씩 묶어서 검사
    for (int i = 0; i < _newmapdata.CityCount; i++)
    {
      _poslist.Clear();
      _poslist.Add(_newmapdata.City_Pos[i*2]);
      _poslist.Add(_newmapdata.City_Pos[i * 2+1]);
      List<Vector3Int> _temp = new List<Vector3Int>();
      foreach (var _asdf in _poslist) _temp.Add(_asdf);
      _poses.Add(_temp);
    }
    _settles = ClassifySettle(_poses, _desertcount_city);
    for (int i = 0; i < _settles.Count; i++)
    {
      _newmapdata.Isriver_city[i] = _settles[i].IsRiver;
      _newmapdata.Isforest_city[i] = _settles[i].IsForest;
      _newmapdata.Ismine_city[i] = _settles[i].IsMine;
      _newmapdata.Ismountain_city[i] = _settles[i].IsMountain;
      _newmapdata.Issea_city[i]= _settles[i].IsSea;
      _newmapdata.Wealth_city[i] = _settles[i].Wealth;
      _newmapdata.Faith_city[i] = _settles[i].Faith;
      _newmapdata.Culture_city[i] = _settles[i].Culture;
      _newmapdata.Science_city[i] = _settles[i].Science;
    }
    _poses.Clear();
    _poslist.Clear();
    //성채 3개씩 묶어서 검사
    for (int i = 0; i < _newmapdata.CastleCount; i++)
    {
      _poslist.Clear();
      _poslist.Add(_newmapdata.Castle_Pos[i * 3]);
      _poslist.Add(_newmapdata.Castle_Pos[i * 3+ 1]);
      _poslist.Add(_newmapdata.Castle_Pos[i * 3 + 2]);
      List<Vector3Int> _temp = new List<Vector3Int>();
      foreach (var _asdf in _poslist) _temp.Add(_asdf);
      _poses.Add(_temp);
    }
    _settles = ClassifySettle(_poses, _desertcount_castle);
    for (int i = 0; i < _settles.Count; i++)
    {
      _newmapdata.Isriver_castle[i] = _settles[i].IsRiver;
      _newmapdata.Isforest_castle[i] = _settles[i].IsForest;
      _newmapdata.Ismine_castle[i] = _settles[i].IsMine;
      _newmapdata.Ismountain_castle[i] = _settles[i].IsMountain;
      _newmapdata.Issea_castle[i] = _settles[i].IsSea;
      _newmapdata.Wealth_castle[i] = _settles[i].Wealth;
      _newmapdata.Faith_castle[i] = _settles[i].Faith;
      _newmapdata.Culture_castle[i] = _settles[i].Culture;
      _newmapdata.Science_castle[i] = _settles[i].Science;
    }
   ...
   //   Debug.Log(_str);
    ...
    List<Settlement> ClassifySettle(List<List<Vector3Int>> _poses,int _desertcount)
    {
    ...
    #endregion
  //  OutputSettleData(_newmapdata);
    MakeTilemap(_newmapdata);
        return _newmapdata;
    }
  public void OutputSettleData(MapData _data)
  {
    string _str = "";
  ...
  public void MakeTilemap(MapData _data)
    {
        Debug.Log("맵을 만든 레후~");
        //타일로 구현화
        for (int i = 0; i < DefaultSize; i++)
        {
            for (int j = 0; j < DefaultSize; j++)
            {
                ...
                Matrix4x4 _trans = Matrix4x4.Translate(Vector3.zero) * 
                    Matrix4x4.Rotate(Quaternion.Euler(new Vector3(0, 0, -60.0f * _data.RotCode[i * DefaultSize + j])));
                Tile _targettile= GetTile_bottom(_data.BottomTileCode[i * DefaultSize + j]);
                TileChangeData _tiledata = new TileChangeData
                {
                    position = _newpos,
                    tile = _targettile,
                    color = Color.white,
                ...
                Tilemap_bottom.SetTile(_tiledata, false);

                Tilemap_top.SetTile(_newpos,GetTile_top(_data.TopTileCode[i * DefaultSize + j]));
            }
        }
    }
    public Tile GetTile_bottom(int index)
    {
        return MyTiles.GetTile_bottom(index);
    }
    public Tile GetTile_top(int index)
    {
